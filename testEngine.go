...
const txSleepMinimum = time.Millisecond * 1175
...
func catchSigterm(sigs chan os.Signal) {
	_ = <-sigs

	run = false

	if logFile, err := os.Create("txLogN"+os.Args[1]); err == nil {
		if _, err := logFile.Write([]byte(fmt.Sprintf("Txs Generated by engine %s: %d", os.Args[1], txs))); err != nil {
